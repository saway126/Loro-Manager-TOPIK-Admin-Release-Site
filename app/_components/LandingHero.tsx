...
> = ({ variant = 'primary', children, ...props }) => {
  const baseClasses =
    'inline-flex items-center justify-center gap-2 px-6 py-3 font-semibold rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 text-base shadow-soft-lg';
  const variantClasses = {
    primary: 'bg-primary-DEFAULT text-white hover:bg-primary-dark focus:ring-primary-light',
    secondary:
      'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 focus:ring-slate-400',
  } as const;
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-24 sm:py-32 text-center">
        ...
        <p className="mt-6 max-w-2xl mx-auto text-lg text-slate-600 dark:text-slate-300">
          모의고사부터 AI 스피킹 훈련까지, LORO는 TOPIK 수험생과 교육자를 위한 가장 완벽한 올인원
          솔루션입니다.
        </p>
      ...
      <PreviewModal open={open} onClose={() => setOpen(false)} title="LORO 제품 미리보기">
        <ProductPreview />
      </PreviewModal>
    </section>
  );
